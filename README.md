...
The operation still takes more than calculated, since the amount of operations needs to be multiplied by the memory reads per operation, which has not varied, although we have removed the useless memory transactions by not using padding. This added time is reflected also in the results we have already commented on, where we have seen that the new method does not in fact surpass the old one, when it is tested in the best possible conditions (like this case).
